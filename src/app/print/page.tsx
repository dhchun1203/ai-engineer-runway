...
const CARD_CLASS =
  "card-interactive flex flex-col gap-1 rounded-xl border border-badge-neutral-bg p-4 dark:border-badge-neutral-bg-dark";
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-10 px-4 py-12 sm:px-6 lg:px-8">
      <header className="flex flex-col gap-3">
        <h1 className="text-display font-bold">PDF 내보내기</h1>
      ...
      {/* 실제로 손이 어디로 가야 하는지 단계로 적는다 — "인쇄하세요" 한 줄로는
          공유 시트까지 도달하지 못한다. */}
      <section className="flex flex-col gap-3 rounded-xl border border-badge-neutral-bg p-4 dark:border-badge-neutral-bg-dark">
        <h2 className="text-subhead font-semibold">아이패드에서 Notability로 넘기는 법</h2>
      ...
      {allScope ? (
        <section className="flex flex-col gap-3">
          <h2 className="text-heading font-bold">한 번에 전부</h2>
          <Link
            href={`/print/${allScope.slug}`}
            className={`${CARD_CLASS} border-l-4 border-l-accent dark:border-l-accent-dark`}
          >
            <span className="flex items-center gap-2 text-subhead font-semibold">
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-heading font-bold">Step별</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {stepScopes.map((scope) => (
            ...
            >
              ...
              <span className="text-subhead font-semibold">{scope.title}</span>
              <span className="text-label font-normal font-mono text-badge-neutral-text dark:text-badge-neutral-text-dark">
      ...
      <section className="flex flex-col gap-6">
        <h2 className="text-heading font-bold">모듈별</h2>
        {steps.map((step) => {
          const scoped = moduleScopes.filter((scope) => scope.stepId === step.id);
          if (scoped.length === 0) return null;
          return (
            <div key={step.id} className="flex flex-col gap-3">
              <h3 className="text-subhead font-semibold text-badge-neutral-text dark:text-badge-neutral-text-dark">
