...
| 소개(/about, Making-of) 페이지 작성 방식: `eli5` 스킬 활용 (2026-08-25 사용자 지시) | 기술 용어 나열이 아니라 "아무것도 모르는 사람 눈높이 + 큰 그림 + 적은 글"로 사이트가 만들어진 과정을 설명 | ○ Pending — `docs/making-of.md`(PLAT-03 단일 소스)를 `Skill(eli5:eli5)` 접근으로 재작성/갱신. Phase 3 종료 후 quick task로 1차 적용, 이후 phase 전환마다 같은 방식으로 갱신 |
