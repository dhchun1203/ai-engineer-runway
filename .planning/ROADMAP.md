...
- [ ] **Phase 8: 성능·인터랙션·스마트폰 최적화** - 정적 생성 전환·폰트 축소와 함께 버튼·인터랙션 감각을 다듬는다
...
- [x] 06-08-PLAN.md — 375px 오버플로 게이트 신설 + 임의값 게이트 활성화 + 게이트 13종 스위트 + UAT 기록 + 시각 검수 체크포인트 (SC1~SC4)
...
**Goal**: 아이패드와 폰 모두에서 페이지가 즉시 열리고, 누르고 넘기는 감각이 "직접 만든 티"가 아니라 잘 다듬어진 제품처럼 느껴진다 — 콘텐츠 페이지를 요청마다
...
**Success Criteria** (what must be TRUE):
  ...
  5. 버튼·카드·토글에 누르는 감각이 있고(현재는 hover만 있고 :active 피드백이 없다), 로딩·빈
     상태·에러 상태가 각각 제 모습을 갖는다 — 아이패드에서 스크롤·전환이 끊기지 않는다

---

**인터랙션 작업 방침 (2026-08-27 결정, 사용자와 논의 후)**

사용자 요청: "인터렉션 요소·버튼 요소 등 트렌디한 디자인 기법을 적용하고 싶다 — 단순히 디자인
감사가 아니라." 즉 `/gsd-ui-review`(감사)가 아니라 실제 적용이 목적이다.

**쓸 스킬: `design-taste-frontend` (부분 적용).** 이 스킬은 감사가 아니라 적용용이고 인터랙션
부분이 구체적이다 — 4.5절(누를 때 `translate-y-[1px]`/`scale-[0.98]`로 물리적 눌림, 스피너 대신
레이아웃 모양을 닮은 스켈레톤, 빈/에러 상태, 버튼 대비·줄바꿈 하드 룰), 5.A~C절(sticky-stack,
가로 팬, 순차 등장 뼈대 코드), 5.D절(하지 말 것: `window.addEventListener("scroll")` 금지,
스크롤 위치를 React 상태로 추적 금지 — 프레임마다 리렌더), 6절(`transform`/`opacity`만
애니메이트, 움직임 줄이기 설정 존중 필수).

**부분 적용인 이유:** 스킬 스스로 첫 줄에 "랜딩 페이지·포트폴리오·리디자인용, 대시보드·데이터
표·다단계 제품 UI는 아님"이라고 범위를 긋는다. 이 사이트는 학습 대시보드에 가깝다. 다만 같은
자리에 "모든 규칙은 상황에 따르며 자동 적용되지 않는다, 브리프를 먼저 읽고 맞는 것만 가져와라"고도
적혀 있다. → 인터랙션·버튼·성능 부분(4.5/5/6절)은 가져오고, 히어로·후기 카드·CTA 의도 중복 같은
랜딩 전용 규칙은 버린다.

**성능과 한 페이즈로 묶은 이유:** 인터랙션은 성능과 직결된다. 잘못 만들면 스크롤이 끊기고 그건
아이패드에서 바로 체감된다. 스킬의 5.D·6절이 정확히 그 얘기를 하고 있어 따로 하면 같은 내용을
두 번 다룬다.

**Motion 라이브러리는 조건부.** 스킬은 애니메이션 기본값으로 Motion(구 Framer Motion)을 전제하지만
이 프로젝트는 지금까지 "새 패키지 없이"를 지켜왔고 의존성이 매우 적다(약 30~50KB 추가). 원하는 것의
상당수 — 버튼 눌림, hover, 포커스, 트랜지션, 순차 등장 — 는 CSS만으로 된다. **CSS로 되는 것을 먼저
하고, Motion은 스크롤 연동 물리나 요소 간 공유 전환처럼 실제로 필요한 지점이 생겼을 때만 넣되 이유를
남긴다.**

**검증 방침:** 아이패드에서 스크롤·전환이 60fps를 유지하는지 자동 게이트로 확인한다 — "보기에
...
- [x] 08-01-PLAN.md — Wave 0 검증 하네스 3종: 라우트 렌더 모드 계약 게이트, 프로덕션 빌드 기반 TTFB·전송량·프레임 예산 게이트, 폰트 글리프 커버리지 게이트

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 08-02-PLAN.md — **트레이서**: `/step/[stepId]` 정적 전환 수직 슬라이스 (`GET /api/progress` + 진도 아일랜드 + 스켈레톤) + G9·G14·G17 재정렬 + G21 신설

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 08-03-PLAN.md — `/lesson/[lessonId]` 정적 전환 + 완료·메모 아일랜드 + 메모 빈 값 덮어쓰기 방지
- [x] 08-04-PLAN.md — 첫 방문 폰트 전송량 측정 → 임계값 대조 → (조건 충족 시) 서브셋 생성·교체
- [x] 08-05-PLAN.md — 폰(375px) 가독성 정량 게이트 신설 + 일정표 행 재배치(아이패드 배치 불변)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 08-06-PLAN.md — `/curriculum` 정적 전환 + 브라우저 계산 D-day + `/`·`/schedule` 동적 유지 결정 기록
...
- [x] 08-08-PLAN.md — 게이트 19종 전량 실행 + 배포 + 측정 종합표 + Making-of 갱신 + 아이패드·폰 실기기 UAT

**Waves:** 1: 08-01 · 2: 08-02 · 3: 08-03·08-04·08-05(병렬) · 4: 08-06 · 5: 08-07 · 6: 08-08

**계획 단계에서 확정한 주요 결정:**

- ISR(`revalidate`)을 어느 라우트에도 쓰지 않는다 — 재검증 창 만료 후 첫 요청이 낡은 페이지를
