...
## 후속 개선 (같은 quick, 사용자 리뷰 반영)

배포 후 사용자 리뷰로 3건 추가 구현·검증(37b9c6d, ada7a93):

1. **"로그인 정보 저장" 체크박스**(remember-me): 체크(기본)=지속 로그인(재시작해도 유지),
   해제=세션 전용(브라우저 닫으면 만료). 동반 쿠키 `sb-persist`(1|0)로 로그인 액션과 proxy
   리프레시가 지속성을 일관 유지 — 해제 시 인증 쿠키에서 maxAge/expires 제거. 내장 브라우저
   실측: 체크→sb-persist=1, 해제→sb-persist=0.
2. **내비 라벨 동적화**: "계정" → 비로그인 "로그인" / 로그인 "프로필". 루트 레이아웃이 쿠키를
   읽으면 정적 셸이 동적 강등되므로, 클라이언트에서 신규 경량 라우트 `/api/auth`(hasUnlockCookie만)
   를 경로 변경 시 조회해 라벨 결정. 로그아웃은 홈('/')으로 보내 라벨 재조회 유도(/login→/login
   동일 경로 미갱신 버그 수정). 실측: 로그인→프로필, 로그아웃→로그인 전환 확인.
3. **로그인 화면 정렬**: 좌우는 원래 중앙정렬(측정상 좌우 여백 동일). 세로 중앙정렬(justify-center)이
   헤더 아래 빈 공간을 만들어 "쏠려" 보이던 것을 상단 정렬로 교체(다른 페이지와 일관).

프로덕션 배포·로그인 왕복 실측 완료(ai-engineer-runway.vercel.app) — Vercel 환경변수
(OWNER_EMAIL·SUPABASE_ANON_KEY) 사용자 추가 후 로그인/로그아웃 정상 동작 확인.
