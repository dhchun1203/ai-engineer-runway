...
Plan: —
Status: Awaiting next milestone
Last activity: 2026-09-06 — 책으로 읽기(텍스트 리더) 기능 다시 활성화(커밋 be3a131, b55fe54 revert의 revert, 사용자 요청): /book/[step] 리더·book-scopes·velite bookCode·책 책갈피·읽기 진행 바 복원. next build 성공(/book/1·2·3 재생성). **음성 기능은 제거 상태 유지**(narration 0, Supabase book-audio 버킷 삭제됨). book_bookmark 테이블은 그대로 있어 책갈피 동작. 복습 객관식 210문항도 유지.
...
| 260904-2wd | 커리큘럼 목록에서 "더 공부할 레슨" 한눈에 — Step 페이지 모듈 아코디언의 표시 레슨 줄에 🚩 마크(완료 ✓와 공존)·모듈 헤더 "더 공부 N" 개수 배지(아코디언 안 열어도 보임), 커리큘럼 StepCard에 Step 단위 "더 공부 N" 배지. 데이터: readNeedsReviewLessonIds→/api/progress needsReviewSlugs(completedSlugs 동형·실패 시 null 강등), 각 UI가 아는 슬러그와 교집합해 카운트. 공용 표시자(action 색으로 완료와 구분). StepCard는 서버가 넘긴 stepLessonSlugs 사용(클라 매니페스트 미접촉). build·게이트(신규 실패 0) 통과, 읽기 경로 curl end-to-end 확인 | 2026-09-04 | 4e6f62d, 9391ac2 | [260904-2wd-review-lessons-overview](./quick/260904-2wd-review-lessons-overview/) |
| 260904-a1o | 스탭별 "책으로 읽기"(/book/[step]) — 스텝 화면 "📖 책으로 읽기" 버튼 → 그 스텝 레슨을 한 권의 책처럼 이어 읽기. 표지 → 스텝 여는 글 → 챕터(레슨)들이 왜 배우나 → 개념 설명 → NextTeaser(다음 챕터 다리)로 흐르고 ❧로 구분, 학습 장치(목표·코드실행기·팁·용어표·점검)는 걷어낸 bookCode 사용. velite에서 고정 "## N" 헤딩으로 원문을 잘라 @mdx-js/mdx function-body로 bookCode·bookMinutes 생성(렌더는 기존 MDXContent 그대로), book-scopes.ts 조립, print/[scope]와 같은 정적 프리렌더(SSG /book/1·2·3, 진도·쿠키 안 읽음), rAF 진행 바. remark-gfm 직접 의존성 추가. 아이패드 768 라이트/다크 실측·build·게이트·lint 통과. 곁다리로 G22 스크롤 리스너 허용목록을 검증형으로 일반화해 선행 bookmark-button 드리프트도 초록화. Step 1 파일럿, 2·3 심화는 승인 후. 추가: 책갈피 한 개(ee5a49d, 챕터 앵커) → 기기 간 동기화(fb21105, localStorage→Supabase book_bookmark·단일 소유자·RLS·service_role, 로그인 시 FAB 표시) + 모바일 햄버거 메뉴 밀어내기→오버레이(465dd94, absolute top-full·내부 스크롤) | 2026-09-04 | 70ea9ae, 7a991cf, ee5a49d, fb21105, 465dd94 | [260904-a1o-book-reader-book-scope-step-1](./quick/260904-a1o-book-reader-book-scope-step-1/) |
| 260905-vbc | 책으로 읽기 음성 플레이어(Phase C, Step 1) — 무료 오픈소스 TTS(XTTS, 코랩 무료 GPU)로 만든 Step 1 챕터 10개 MP3(~25분)를 Supabase Storage 공개 버킷 book-audio/step1에 업로드, /book/[step]에 하단 재생 바. src/content/book-audio.ts 매니페스트(slug로 book-scopes 챕터 매칭, 없는 스텝 undefined→미렌더), book-audio-player.tsx(client, <audio> 하나로 챕터 이어재생·자동다음·10초앞뒤·이전/다음·배속·진행막대·챕터목록·재생중 본문 챕터 스크롤+강조·이어듣기 localStorage·MediaSession 잠금화면), globals.css 하단 바+FAB 밀어올림(메모장 :has() 패턴 재사용, 겹침 방지). 내장 브라우저 768 검증(재생·점프·강조·이어듣기·라이트/다크·콘솔0), tsc·eslint 통과. Step 1 파일럿, 2·3은 같은 노트북 재실행 후 확장 | 2026-09-05 | 7dd3469 | [260905-vbc-book-step-step1](./quick/260905-vbc-book-step-step1/) |
| 260904-in4 | 책으로 읽기 낭독(TTS) Phase A — 책 본문(sliceBookContent)에서 순수 한국어 문장 배열 narration을 빌드타임에 추출(다음 Phase 음성 생성이 소비, 오디오·API·플레이어는 아직 없음). stripBlocks 공용 헬퍼로 SVG·코드펜스·태그 제거를 estimateBookMinutes와 공유(드리프트 구조적 차단, bookMinutes 출력 불변), extractNarration은 GFM 표 스킵·헤딩/리스트/인용/장식이모지 제거·인라인 기호만 제거(단어 내부 하이픈 보존)·한글 종결 룩비하인드로 문장 분리(소수점 3.14·1.0 미분절). lessons 스키마 narration 필드(terms/selfCheck와 같은 hasContent 게이트), scripts/check-narration.mjs 상시 게이트(잔여물·소수점 분절·빈 대본). velite build 통과, 35레슨 전부 대본 764문장, 파일럿(Python 변수·자료형) 30문장 육안 확인 잔여물 0 | 2026-09-04 | 86f00d9 | [260904-in4-phase-a-velite-narration](./quick/260904-in4-phase-a-velite-narration/) |
