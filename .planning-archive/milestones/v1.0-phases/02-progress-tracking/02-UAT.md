...
result: pass
source: automated
evidence: "로컬 dev 서버를 SUPABASE_URL=무효 호스트로 기동해 홈·/step/1·레슨 3면 확인 — data-progress-ui="read-error" 배너만 1개씩 렌더, 그 외 progress-ui 마커 0개, 진행률 수치/0% 미노출(레슨 페이지 % 문자열은 코드블록 CSS 속성값)"
...
result: pass
source: automated
evidence: "ProgressSummary에 completed=total=35, percent=100 + nextLessonSlug 주입 렌더 — 제목 '커리큘럼을 모두 완료했어요!', CTA '커리큘럼 처음으로'(/step/1) 교체, '이어서 학습하기'/다음 레슨 링크 미노출. 대조군(2/35)은 '이어서 학습하기' 정상"
